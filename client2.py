...
try:
    ...
    # Avoid using messagebox before Tk() is initialized
    print("Missing Dependency: Pillow (PIL) is required. Please install it with 'pip install pillow'.")
    raise
import sys
...
class PokerClient:
    def __init__(self):
        ...
        self.player_id = None  # Will be set on connect
        self.player_name = ""
        ...
        self.player_widgets = {}
        self.my_card_labels = []  # Initialize to avoid AttributeError
    ...
    def load_card_images(self):
        # This method is intentionally left blank or can be used for future refactoring.
        pass
    ...
    def setup_connection_dialog(self):
        ...
        def connect():
            ...
            self.player_id = str(uuid.uuid4())  # Ensure unique ID per client connection
            if self.connect_to_server(server_ip):
                dialog.destroy()
                # Create Start button after dialog is closed and connection is successful
                self.start_btn = tk.Button(
                    self.root, text="Start New Hand", command=self.start_game,
                    bg='#27AE60', fg='white', font=('Arial', 14, 'bold'), padx=20, pady=10
                )
            else:
        ...
        # Tombol Start akan dibuat setelah dialog koneksi ditutup
        self.start_btn = None
    ...
    def handle_server_message(self, message):
        ...
        if msg_type == 'join_success':
            ...
            # Prevent double notification by only showing the winner message here
            ...
            if 'players' in self.game_data:
                ...
                # Use a flag to prevent double notification
                if not hasattr(self, '_winner_shown') or not self._winner_shown:
    ...
    def update_players(self, players_data):
        ...
        for pid, p_data in players_data.items():
            ...
            # Ensure p_data is a dict before accessing keys
            ...
            cards_frame.pack(padx=5, pady=(2,5))
            # Keep references to images to prevent garbage collection
            opponent_card_labels = []
            # Only show real cards if they are revealed (e.g., at showdown), otherwise show card backs
            show_real_cards = p_data.get('show_cards', False)
            cards = p_data.get('cards', [])
            cards_list = cards if isinstance(cards, list) else []
            cards_list = cards_list[:2]
            try:
                num_cards = min(len(cards_list), 2)
            except Exception:
                num_cards = 0
            for i in range(num_cards):
                card_image = self.card_back_image
                card_data = cards_list[i] if i < len(cards_list) else None
                if show_real_cards and isinstance(card_data, dict) and card_data.get('image'):
                    card_image = self.card_images.get(card_data.get('image', ''), self.card_back_image)
                else:
                    card_image = self.card_back_image
                if card_image is None:
                    card_image = self.card_back_image
                label = tk.Label(cards_frame, image=card_image, bg='#111111')
                label.image = card_image  # Keep a reference on the label
                label.pack(side=tk.LEFT, padx=1)
                opponent_card_labels.append(label)
    
    def update_my_cards(self, cards):
        for widget in self.my_cards_display_frame.winfo_children():
            widget.destroy()
        self.my_card_labels = []  # Keep references to prevent garbage collection
        for card_data in cards:
            card_image = self.card_images.get(card_data['image'], self.card_back_image)
            if card_image:
                label = tk.Label(self.my_cards_display_frame, image=card_image, bg='#111111')
                label.image = card_image  # Keep a reference on the label
    ...
    def update_action_buttons(self, is_my_turn):
        ...
        # Always reset call button text and state
        ...
        if not my_data or my_data.get('is_folded') or my_data.get('is_all_in'):
            return
        ...
        try:
            current_bet = int(current_bet) if current_bet is not None else 0
        except (TypeError, ValueError):
            current_bet = 0
        try:
            my_bet = int(my_bet) if my_bet is not None else 0
        except (TypeError, ValueError):
            my_bet = 0
        try:
            my_chips = int(my_chips) if my_chips is not None else 0
        except (TypeError, ValueError):
            my_chips = 0
        ...
        if current_bet > my_bet:
            ...
            if my_chips >= call_amount:
                ...
                self.call_btn.config(state=tk.NORMAL, text=f"All-in ${my_chips}")
        else:
            self.check_btn.config(state=tk.NORMAL)
            self.call_btn.config(state=tk.NORMAL, text="Call")
    ...
    def call_action(self):
        ...
        if not my_data:
            return
        try:
            my_current_bet = int(my_data.get('current_bet', 0))
            my_chips = int(my_data.get('chips', 0))
        except (TypeError, ValueError):
            my_current_bet = 0
            my_chips = 0
        if my_chips <= (current_bet - my_current_bet):
...
def show_winner_and_exit(winner, root):
    messagebox.showinfo("Game Over", f"{winner} wins!")
    root.destroy()  # This will close the Tkinter window
    sys.exit()      # This will forcefully exit the program

if __name__ == '__main__':
    client = PokerClient()
    client.run()
    # After the mainloop ends (e.g., after a winner is shown), force close the client
    sys.exit(0)